...
clouds = ["ifb-bird-stack", "ifb-bistro-iphc", "ifb-core-pilot", "ifb-genouest-genostack", "ifb-prabi-girofle"]

url_template = "https://nuv.la/api/usage?$filter=cloud='{cloud}' and frequency='monthly' and start-timestamp=2017-{month}-01"
...
def login():
    r = requests.post(url_login, data={"username":username, "password":password})
...
def cloud_loop(month):
    ...
    for cloud in clouds:
        ...
        response = requests.get(url, cookies=cookies)#auth=(username, password))
        test_response_raise(response, "Error getting usage: " + response.text)
        clouds_metrics[cloud] = process_json(response.json())
...
def format(clouds_metrics, filename):
    ...
    with open(filename,'w') as f:
        f.write("Cloud, VM (hours), CPU (hours), RAM (GB-hours), Disk (GB-hours), Active users\n")
...
def merge_files():
    filenames = map(lambda m: 'metrics-%s.csv' % pad_filename(m), range(1,13))
